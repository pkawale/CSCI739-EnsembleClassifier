...
class RandomForest:
    ...
    def parallel_tree_build(self, dataset):
    ...
    def fit(self, dataset):
        with concurrent.futures.ThreadPoolExecutor() as executor:
            trees_indices = [i for i in range(self.num_trees)]
            trees = list(executor.map(self.parallel_tree_build, dataset))
        self.parallel_tree_build(np.array(dataset))
...
def load_data(ip = 'iris'):
    ...
    if(ip == 'iris'):
        ...
        print("Unkonwn data load request")
        sys.exit(1)
...
def main():
    ...
    rf = RandomForest(num_trees=10, min_samples_split=3, max_depth=15, max_features=None)
